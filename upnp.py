...
class UPNPResponderThread(threading.Thread):
    ...
    def __init__(
        ...
        resp_template_rootdevice = f"""HTTP/1.1 200 OK
...
"""
        self.upnp_response_rootdevice = resp_template_rootdevice.replace(
            "\n", "\r\n"
        ).encode("utf-8")

        resp_template_fallback = f"""HTTP/1.1 200 OK
CACHE-CONTROL: max-age=60
EXT:
LOCATION: http://{advertise_ip}:{advertise_port}/description.xml
SERVER: FreeRTOS/6.0.5, UPnP/1.0, IpBridge/1.16.0
...
"""
        self.upnp_response_fallback = resp_template_fallback.replace(
            "\n", "\r\n"
        ).encode("utf-8")
        ...
        while True:
            ...
            if "M-SEARCH" in data.decode("utf-8", errors="ignore"):
                ...
                if "upnp:rootdevice" in data.decode("utf-8", errors="ignore"):
                    resp_socket.sendto(self.upnp_response_rootdevice, addr)
                else:
                    resp_socket.sendto(self.upnp_response_fallback, addr)
